...
module Number {
  ...
  const MIN_I24  : int := -TWO_23
  const MAX_I24  : int :=  TWO_24 - 1
  ...
  newtype{:nativeType "int"}   i24 = i:int  | MIN_I24 <= i <= MAX_I24
...
module I256 {
  ...
  function Rem(lhs: i256, rhs: i256) : i256
    // Cannot divide by zero!
    requires rhs != 0 {
    Number.Rem(lhs as int, rhs as int) as i256
  }
...
module Word {
  ...
  // question represents an unsigned 256 bit integer.  Rather, it is a
  // signed integer encoded into an unsigned integer.
  function fromI256(w: Number.i256) : u256 {
    if w < 0
    then
      var v := 1 + MAX_U256 + (w as int);
      v as u256
